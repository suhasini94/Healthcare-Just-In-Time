...
(function ($) {
    ...
    //////////////////private functions//////////////////////////////////////////
    // functions for symptom selector on far right //
...
(function ($) {
    ...
    function _createRedFlagMessage() {
        ...
        // Functionality to call ambulance when symptoms are dire
        call = jQuery("<a/>", { "text": " Please call emergency assistance if condition is serious: " });
        emergency = jQuery("<a/>", { "href": "tel:999", "text": "999" });
        ...
        content.append(message).append(call);
...
(function ($) {
    ...
    // When 'i' button is clicked, more information is provided
    // upon closing the info page, pop up appears asking user 
    // whether app should book doctors appointment for them 
    function _createIssueInfoPage() {
        ...
        _infoPage.append(infoPageLoader);
         ...
         // Pop up for asking whether appointment booking is necessary upon closing info page
         modal = jQuery("<p/>", { "id": "modal" });
         var txt;
        ...
        btnClose.bind('click', function () {
            ...
            // pop up 
            if (confirm("Would you like to book an appointment with a specialist?")) {
                if(confirm("Would you like to book a Skype appointment?")){ 
                    // appt may be booked on your calendar as well as
                    // doctor's calendar if ID is known via Cronofy API
                    var Cronofy = require('cronofy');
                    var client = new Cronofy({ access_token: "FFxURhVBSeroIhGStHIbgsXPC6pRe65U"});

                    var options = { calendar_id: "cal_W6@erqrHdBSDw6JW_p135U8T470MkBOHddb4zmQ",
                                      event_id: "unique-event-id",
                                      summary: "Specialist Appointment",
                                     description: "Appointment with specialist.",
                                     start: "2018-10-02T12:00:00Z",
                                     end: "2018-10-02T12:30:00Z",
                                     location: {
                                      description: "hospital"
                                     }
                                  };

                    client.createEvent(options) .then(function () {   // Success
                 });
                 };
                 } else {
